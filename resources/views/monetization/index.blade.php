<x-app-layout :title="__('Monetization')">
    ...
    @if (! $allowed)
        <div class="lf-card flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <p class="mt-1.5 max-w-md text-sm text-slate-500">{{ __('On a paid plan, our ads are removed from your links and you can place your own ad-network code (Google AdSense, etc.) on your links, keeping 100% of the revenue.') }}</p>
    ...
    @else
        <div class="grid gap-6 lg:grid-cols-[1fr_320px]">
            <form method="POST" action="{{ route('monetization.update') }}" class="lf-card p-6">
                @csrf @method('PUT')
                <h3 class="text-sm font-semibold text-slate-900">{{ __('Your ad code') }}</h3>
                <p class="mt-1.5 text-sm text-slate-500">{{ __('Paste an ad-network snippet (for example Google AdSense). It runs on a short interstitial page when someone opens one of your links, so you earn from your own traffic. Leave blank to show no ad.') }}</p>
                <textarea name="ad_code" rows="9" class="lf-input mt-4 font-mono text-xs" placeholder="&lt;script async src=&quot;https://pagead2.googlesyndication.com/...&quot;&gt;&lt;/script&gt;">{{ old('ad_code', $adCode) }}</textarea>
                @error('ad_code')<p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>@enderror
                <div class="mt-4 flex justify-end">
                    <button type="submit" class="lf-btn">{{ __('Save ad code') }}</button>
                </div>
            </form>
            ...
            <div class="lf-card h-fit p-6 text-sm text-slate-600">
                ...
                <ul class="mt-3 space-y-2.5">
                    ...
                    <li class="flex gap-2"><span class="text-brand-600">&bull;</span> {{ __('Your code is shown instead, on your links interstitial page.') }}</li>
                    <li class="flex gap-2"><span class="text-brand-600">&bull;</span> {{ __('It runs sandboxed for safety, so most banner/display tags work; some interactive formats may not.') }}</li>
                    <li class="flex gap-2"><span class="text-brand-600">&bull;</span> {{ __('Direct links still redirect instantly when you have no ad code set.') }}</li>
                </ul>
            </div>
        </div>
    @endif
