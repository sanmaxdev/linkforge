...
@if ($lfPopAd)
    <div id="lf-ad-popup" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm">
        <div class="relative w-full max-w-md rounded-2xl bg-white p-5 shadow-xl">
            <button type="button" data-lf-ad-close aria-label="Close" class="absolute right-3 top-3 rounded-md p-1 text-slate-400 transition hover:bg-slate-100 hover:text-slate-600">
    ...
    <script>
        (function () {
            ...
            if (!el) return;
            function close() { el.classList.add('hidden'); el.classList.remove('flex'); sessionStorage.setItem(key, '1'); }
            setTimeout(function () { el.classList.remove('hidden'); el.classList.add('flex'); }, 1200);
            el.addEventListener('click', function (e) { if (e.target === el) close(); });
            el.querySelector('[data-lf-ad-close]').addEventListener('click', close);
        })();
    </script>
@endif
